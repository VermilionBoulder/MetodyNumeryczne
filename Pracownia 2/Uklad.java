...
public class Uklad {
    ...
    //nieujemny parametr alfa pozwala zwiekszyc dominacje na przekatnej
    public void LosujUkladSymetrycznyDodatnioOkreslony(double alfa){
        Random rand = new Random();
        double los, suma;
        for(int i = 0; i < n; i++) {
            for(int j = i+1; j< n; j++) {
            ...
            for(int j = 0; j < n; j++) suma += Math.abs(A[i][j])*(1 + alfa*rand.nextDouble());
            A[i][i] = suma;
        }
    }
