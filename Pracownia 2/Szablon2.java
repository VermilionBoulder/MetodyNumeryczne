/**
 *  Klasa rysujaca szablon wykresu dla dwoch list danych
 */
...
public class Szablon2 extends JPanel {
    ...
    double[] T;     //tablica danych - 1
    double[] S;     //tablica danych - 2
    int N_T;        //liczba wyswietlanych danych dla danych 1
    int N_S;        //liczba wyswietlanych danych dla danych 2
    int N;          //wieksza z liczb N_T oraz N_S
    
    /*Konstruktor przyjmujacy:
     *  dane1, dane2 - listy danych
     *  n0 - numer pierwszej wyswietlanej danej
    */
    public Szablon2(java.util.List dane1, java.util.List dane2, int n0) {
        width = 800;
        height = 600;
        N_T = dane1.size() - n0;
        N_S = dane2.size() - n0;
        ...
        for(int i = 0; i < N_T; i++) {
            n[i] = n0 + i;
            T[i] = (double) dane1.get(n[i]);
        }
        for(int i = 0; i < N_S; i++) {
            n[i] = n0 + i;
            S[i] = (double) dane2.get(n[i]);
        }
    ...
    public void getMax(){
        ...
        for(int i = 0; i < N_S; i++) {
            if(S[i] > MAX_T) MAX_T = S[i];
            if(S[i] < MIN_T) MIN_T = S[i];
        }
    }
    ...
    protected void paintComponent(Graphics g){
        ...
        for(int i = 0; i<N_S; i++){
            punkt.setFrame(scn*(n[i]-MIN_n) + 5*dist-3,height-4*dist-scT*(S[i]-MIN_T)-3,6,6);
            g2d.fill(punkt);
            g2d.draw(punkt); 
        }   
    }
