/**
 *  Klasa realizujaca metode iteracji prostej
 */
...
public class IteracjaProsta {
    ...
    public IteracjaProsta(Uklad uk) {
        n = uk.n;
        u = new Uklad(uk);
    }
    ...
    public void Przygotuj(){
        ...
        //okreslamy macierz D i wektor C
        for(int i = 0; i < n; i++){
    ...
    public void Iteruj(int iteracje, int norma, int wyswietl, double[] X0) {
        ...
        while(k < iteracje){
            for(int i = 0; i < n; i++) {
                x[i] = C[i];
                for(int j = 0; j < n; j++) x[i] += D[i][j] * x_st[j];
            }
            k++;
